...
function initFormListeners() {
  ...
  // ── CLIP Campus Search ─────────────────────────────────────
  const formClip = document.getElementById("form-clip-search");
  if (formClip) {
    formClip.addEventListener("submit", async (e) => {
      e.preventDefault();
      const q = document.getElementById("clip-query").value;
      const res = await fetch(`${API_BASE}/multimodal/clip-search`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ query: q })
      });
      const results = await res.json();
      const container = document.getElementById("clip-results-container");
      container.innerHTML = "";
      results.forEach(item => {
        const div = document.createElement("div");
        div.className = "clip-item";
        div.innerHTML = `
          <img src="${item.image_url}" class="clip-img" alt="${item.title}">
          <div class="clip-info">
            <strong>${item.title}</strong><br>
            <span class="text-accent">Similarity: ${item.similarity_score}</span>
          </div>`;
        container.appendChild(div);
      });
    });
    formClip.dispatchEvent(new Event("submit"));
  }

  // ── Vision AI (BLIP + CNN) — Submit button flow ────────────
  const visionInput = document.getElementById("vision-file-input");
  const visionSubmitBtn = document.getElementById("btn-vision-submit");
  ...
  if (visionInput) {
    visionInput.addEventListener("change", (e) => {
      ...
      if (visionSubmitBtn) visionSubmitBtn.style.display = "block";
      document.getElementById("vision-results-container").innerHTML = "";
    });
  }

  if (visionSubmitBtn) {
    visionSubmitBtn.addEventListener("click", async () => {
      if (!selectedVisionFile) return;
      visionSubmitBtn.disabled = true;
      visionSubmitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Analysing\u2026';

      const fd = new FormData();
      fd.append("file", selectedVisionFile);

      const [resCap, resDoc] = await Promise.all([
        fetch(`${API_BASE}/multimodal/blip-caption`, { method: "POST", body: fd }),
        fetch(`${API_BASE}/multimodal/classify-doc`,  { method: "POST", body: fd })
      ]);
      const capData = await resCap.json();
      const docData = await resDoc.json();

      document.getElementById("vision-results-container").innerHTML = `
        <div class="insight-alert" style="background:rgba(99,102,241,0.1);border-color:rgba(99,102,241,0.4);color:#a5b4fc;margin-bottom:12px;">
          <strong>BLIP Visual Image Captioning</strong><br>
          <span>${capData.blip_caption}</span><br>
          <span style="font-size:0.75rem;color:#94a3b8;">Confidence: ${(capData.confidence_score*100).toFixed(1)}% | Tags: ${capData.extracted_tags.join(", ")}</span>
        </div>
        <div class="insight-alert" style="background:rgba(16,185,129,0.1);border-color:rgba(16,185,129,0.4);color:#6ee7b7;">
          <strong>CNN Document Classifier &amp; Verification</strong><br>
          <span>Classified Type: <strong>${docData.classified_document_type}</strong></span><br>
          <span style="font-size:0.75rem;">Status: ${docData.validation_status} (${(docData.confidence*100).toFixed(1)}% Confidence)</span>
        </div>`;

      visionSubmitBtn.disabled = false;
      visionSubmitBtn.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Run Vision AI Analysis';
    });
  }
  ...
  if (btnCard) {
    btnCard.addEventListener("click", async () => {
      const res = await fetch(`${API_BASE}/multimodal/insight-card`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          college_name: "IIT Bombay", branch_code: "CSE",
          match_score: 94.5, avg_fee: 2.5, avg_pkg: 24.5
        })
      });
      const data = await res.json();
      document.getElementById("insight-card-container").innerHTML = data.svg_card;
    });
    btnCard.click();
  }

  // ── Video Storyboard ───────────────────────────────────────
  const btnBoard = document.getElementById("btn-generate-storyboard");
  if (btnBoard) {
    btnBoard.addEventListener("click", async () => {
      const res = await fetch(`${API_BASE}/multimodal/video-storyboard`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      container.innerHTML = "";
      data.storyboard_scenes.forEach(s => {
        const div = document.createElement("div");
        div.className = "highlight-item margin-bottom-sm";
        div.innerHTML = `
          <div class="kpi-value" style="font-size:1.1rem;color:#38bdf8;">Scene ${s.scene}</div>
          <div>
            <strong>${s.title}</strong>
            <p>${s.narration}</p>
            <p class="text-accent" style="font-size:0.75rem;">Visual Prompt: ${s.visual_prompt}</p>
          </div>`;
        container.appendChild(div);
      });
    });
